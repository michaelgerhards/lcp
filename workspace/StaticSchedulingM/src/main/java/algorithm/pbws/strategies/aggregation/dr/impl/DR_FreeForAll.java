...
public class DR_FreeForAll {
    ...
    public DR_FreeForAll(PBWSInit algorithm,
    ...
    public boolean reviseAllocationPlanForNotDirectlyRelatedTasks() {
        ...
        List<Lane> outers = new ArrayList<Lane>(algorithm.getWorkflow().getLanes());

        Collections.sort(outers,
                Comparators.laneLowerStartTimeComparator);
        int i = 0;
        for (Lane outer : outers) {
            ...
            do {
                finalResult = null;
                List<Lane> nextRoundPartners = new ArrayList<Lane>(
                        partners.size());

                for (Lane inner : partners) {
                    if (outer == inner
                            || !algorithm.getWorkflow().existsLane(inner)) {
                        continue;
                    }
                    AggregationResult result;
                    if (inner.getInstanceSize() != outer.getInstanceSize()) {
                        result = dr_Manager_DifferSize
                                .checkDifferentInstanceSize(inner, outer);
                    } else {
                ...
                if (finalResult != null) {
                    ...
                    if (algorithm.getWorkflow().existsLane(
                            finalResult.getThrower())) {
