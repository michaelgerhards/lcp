...
public class DR_ManyToMe {
	...
	public DR_ManyToMe(PBWSInit algorithm,
			DR_Manager_EqualSize dr_Manager_EqualSize,
			DR_Manager_DifferSize dr_Manager_DifferSize, DR_ThrowerGetter dr_ThrowerGetter) {
	...
	/**
	 * combines lanes that have more than one predecessor with no other
	 * successor.
	 */
	public boolean combineManyToMeRelation(String name) {
		...
		for (LaneIndex throwerIndex : throwers) {
			...
			do { // TODO use caching only for valid combinations?
				...
				Set<Lane> predecessors = new HashSet<Lane>(
						thrower.getUmodParents());
				finalResult = null;
				for (Lane catcher : predecessors) {
					AggregationResult result;
					if (thrower.getInstanceSize() != catcher.getInstanceSize()) {
						result = dr_Manager_DifferSize
								.checkDifferentInstanceSize(thrower, catcher);
					} else {
						result = dr_Manager_EqualSize
								.checkEqualInstanceSize(thrower, catcher);
					}
