...
public class BR_LongestLP_Succ_NoGaps_DifferentSize_TScale_FlexDL implements
		Aggregator {

	private final PBWSInit algorithm;

	public BR_LongestLP_Succ_NoGaps_DifferentSize_TScale_FlexDL(
			PBWSInit algorithm) {
		this.algorithm = algorithm;
	}

	public AggregationResult tryMigrateLastPath_ThrowerAfterCatcher_ThrowerScale_DifferentInstanceSize(
		...
		for (int currentPath = 1; currentPath <= maxPath; ++currentPath) {
			...
			SchedulingTaskList throwerLastPath = thrower
					.getUmodLastPath(currentPath);

			ScaledPseudoLane newThrowerLastPath = thrower.tryScalingIdlFlex(
					catcher.getInstanceSize(), throwerLastPath);
			if (newThrowerLastPath == null) {
				continue;
			}

			double throwShift = catcher.getEndTime()
					- newThrowerLastPath.getStartTime();
			...
			double newCombinedCosts = bu
					.getCosts_LP_NoGaps_DifferSize(catcher, thrower,
							newThrowerLastPath, throwerLastPath);

			if (newCombinedCosts <= combinedCosts) {

				double newThrowerGlobalShiftSpace = newThrowerLastPath
						.getRSWflex();

				if (throwShift > newThrowerGlobalShiftSpace) {
					boolean satisfyNewStartTimeFlex = newThrowerLastPath
							.satisfyNewStartTimeFlex(catcher.getEndTime());
