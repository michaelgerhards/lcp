...
import algorithm.pbws.strategies.scaledownphase.ScaleDownPhase;
import statics.util.Debug;
import statics.util.GUIVisualizer;

public class PBWS extends PBWSInit {
	...
	protected void doIt() {
		...
		GUIVisualizer.guiVisualize("FASTEST Scaling for "
				+ getWorkflow().getWorkflowName(), getWorkflow());
		...
		GUIVisualizer.guiVisualize(
				"after Initial Down-Scaling\nbefore Breadth reduction",
				getWorkflow());
		...
		Debug.INSTANCE.println(1,
				"Start 2nd loop: distributeTasksToNeighborsConsideringRSWflex");
		int loop2Cycles = 0;
		do {
			++loop2Cycles;
			if (printPerformance) {
		...
		GUIVisualizer
				.guiVisualize(
						"after Breadth reduction \nbefore Depth reduction 1-to-1 relation",
						getWorkflow());
		...
		GUIVisualizer
				.guiVisualize(
						"after Depth reduction 1-to-1 relation \nbefore Depth reduction 1-to-M relation",
						getWorkflow());
		//
		s = System.currentTimeMillis();
		...
		GUIVisualizer
				.guiVisualize(
						"after Depth reduction M-to-1 relation \nbefore Depth reduction N-to-M O relation",
						getWorkflow());
		...
		GUIVisualizer
				.guiVisualize(
						"after Depth reduction N-to-M I relation \nbefore Depth reduction best fit",
						getWorkflow());
		...
		printer.printLanes();
		// drawLanesUntilDeadline();
		GUIVisualizer.guiVisualize("after Depth reduction � best fit",
				getWorkflow());
...
//		final long tpp = e - s;
		...
		printer.printExecutionTimes(tDownscale, loop1AggregateNeighborsFix,
				loop1AggregateNeighborsFlex, loop2AggregateNeighborsFix,
				loop2DistributeTasksToNeighborsFix,
				loop2DistributeTasksToNeighborsFlex, t1to1, t1toM, tMto1,
				tMtoNO, tMtoNI, tffa, tpp);
		printer.printCosts(cInit, cDownscale, cLoop1, cLoop2, c1to1, cMto1,
				c1toM, cMtoMO, cMtoMI, cffa, cpp);
