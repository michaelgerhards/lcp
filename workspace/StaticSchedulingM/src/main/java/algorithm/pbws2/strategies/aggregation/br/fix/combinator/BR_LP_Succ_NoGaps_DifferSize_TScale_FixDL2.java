...
public class BR_LP_Succ_NoGaps_DifferSize_TScale_FixDL2 implements Aggregator {

    public AggregationResult tryMigrateThrowerPathAfterCatcher_DifferentInstanceSize_ThrowerScale_NoGaps(Lane thrower, Lane catcher, SchedulingTaskList throwerLastPath) {
        if (thrower.getInstanceSize() == catcher.getInstanceSize() || throwerLastPath.getInstanceSize() != thrower.getInstanceSize()) {
            throw new RuntimeException("wrong instance size in LP_Succ_NoGaps_DifferentSize_ThrowerScale_FixedDL");
        }
        if (catcher.getUmodChildren().contains(thrower) || catcher.getUmodParents().contains(thrower)) {
            throw new RuntimeException();
        }

        ScaledPseudoLane newThrowerLastPath = thrower.tryScalingIdlFix(catcher.getInstanceSize(), throwerLastPath);
        ...
        double newCombinedCosts = BillingUtil.getInstance().getNewCombinedCosts_LP_NoGaps_DifferentSize(thrower, catcher, throwerLastPath, newThrowerLastPath);
        if (newCombinedCosts >= combinedCosts) {
            return null;
        }

//        if (!newThrowerLastPath.hasGap()) {
        final double availableThrowerShiftTime = newThrowerLastPath.getRSWfix();
