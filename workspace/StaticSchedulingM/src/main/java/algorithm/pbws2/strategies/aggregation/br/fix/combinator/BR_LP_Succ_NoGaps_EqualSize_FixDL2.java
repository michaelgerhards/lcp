...
public class BR_LP_Succ_NoGaps_EqualSize_FixDL2 implements Aggregator {
     ...
     * @return true, if the thrower is not released. false if the thrower is released or nothing happened
     */
    public AggregationResult tryMigrateThrowerPathAfterCatcher_EqualInstanceSize_NoGaps(Lane thrower, Lane catcher, SchedulingTaskList throwerLastPath) {
        if (thrower.getInstanceSize() != catcher.getInstanceSize() || throwerLastPath.getInstanceSize() != thrower.getInstanceSize()) {
            throw new RuntimeException("LP_Succ_NoGaps_EqualSize_FixedDL wrong instance sizes");
        }
        ...
        double newCosts = BillingUtil.getInstance().getNewCosts_NoGaps_EqualSize(catcher, thrower, throwerLastPath);
