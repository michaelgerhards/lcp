package algorithm.pbws2.strategies.scaledownphase;
...
public class ScaleDownPhase2 {

    private final AbstractAlgorithm algorithm;
    private final Map<SchedulingTask, UnscaledParentsAndPath> unscaledParents;

    public static void scaleDown(AbstractAlgorithm algorithm) {
        ScaleDownPhase2 scaling = new ScaleDownPhase2(algorithm);
        scaling.scaleDown();
    }

    private ScaleDownPhase2(AbstractAlgorithm algorithm) {
    ...
    private Set<SchedulingTask> getUnscaledParents(SchedulingTask child) {
        ...
        if (parents == null) {
            parents = new HashSet<>(child.getParents());
    ...
    private TryForwardResult tryForward(List<SchedulingTask> pcp,
        ...
        Map<SchedulingTask, Double> localTaskStarttimes = new HashMap<>((int) (pcp.size() * 1.5));
    ...
    private void applyScaleIgnoreOthers(InstanceSize instanceSize,
        ...
        for (SchedulingTask ti : scheduleTasks) {
            ...
            if (oldSize != instanceSize) {
                SchedulingTaskList list = new SchedulingTaskList(laneOfTask.getUmodTasks(), instanceSize, algorithm.getWorkflow());
                ScaledPseudoLane lane = new ScaledPseudoLane(list, bestLocalTaskStarttimes, false);
