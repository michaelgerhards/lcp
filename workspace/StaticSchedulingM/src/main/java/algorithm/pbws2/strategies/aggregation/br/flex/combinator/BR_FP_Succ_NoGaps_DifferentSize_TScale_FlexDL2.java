...
public class BR_FP_Succ_NoGaps_DifferentSize_TScale_FlexDL2 implements
        Aggregator {

    private final PBWSInit2 algorithm;

    public BR_FP_Succ_NoGaps_DifferentSize_TScale_FlexDL2(PBWSInit2 algorithm) {
    ...
    public AggregationResult tryMigrateFullPath_ThrowerAfterCatcher_ThrowerScale_DifferentInstanceSize(
        ...
        double newCombinedCosts = bu.getCombinedCosts_DifferentSize(catcher, newThrower, throwShift);
...
//        if (!newThrower.hasGap()) {
        ...
        if (throwShift - availableThrowerShiftTime > Util.DOUBLE_THRESHOLD) {
            return null;
        }
//        } else {
...
//        }
    ...
    public void performCombination(AggregationResult result) {
        ...
        Set<Lane> shifted = thrower.prepareSuccessorsForOwnVertScale(newThrower, throwShift);
        shifted.add(catcher);
        shifted.add(thrower);
