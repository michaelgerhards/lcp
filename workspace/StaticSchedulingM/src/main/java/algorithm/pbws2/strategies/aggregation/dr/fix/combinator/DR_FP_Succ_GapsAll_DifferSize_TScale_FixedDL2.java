...
public class DR_FP_Succ_GapsAll_DifferSize_TScale_FixedDL2 implements Aggregator {

    public AggregationResult tryCombineThrowerAfterCatcher_ThrowerScaleUpGapsAllowed(Lane catcher, Lane thrower,
            ScaledPseudoLane newThrower) {
        ...
        if (throwerStartTime - catcherEndtime > -Util.DOUBLE_THRESHOLD) {
            ...
            double throwerAtuStart = bu.getBillingStartTime(newThrower);
...
//            }
    ...
    public void performCombination(AggregationResult result) {
        ...
        // boolean throwerReleased =
        // !algorithm.getWorkflow().existsLane(thrower);
        //
        // if (!throwerReleased) {
        // throw new RuntimeException("thrower not released! t=" + thrower
        // + " c=" + catcher);
        // }
