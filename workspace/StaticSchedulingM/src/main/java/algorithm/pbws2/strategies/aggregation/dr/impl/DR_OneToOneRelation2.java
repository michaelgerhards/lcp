...
public class DR_OneToOneRelation2 {
    ...
    public DR_OneToOneRelation2(PBWSInit2 algorithm, DR_Manager_EqualSize dr_Manager_EqualSize, DR_Manager_DifferSize dr_Manager_DifferSize) {
...
//			}
            ...
            if (thrower.getInstanceSize() != catcher.getInstanceSize()) {
                result = dr_Manager_DifferSize.checkDifferentInstanceSize(catcher, thrower);
            } else {
                result = dr_Manager_EqualSize.checkEqualInstanceSize(catcher, thrower);
            } // if else
...
//                remap.put(deleted.getId(), existing.getId());
    ...
    private List<LaneIndex> getOneToOneRelation() {
        List<LaneIndex> oneToOneLanes = new ArrayList<>();
